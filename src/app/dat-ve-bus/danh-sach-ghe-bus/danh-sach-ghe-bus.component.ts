...
export class DanhSachGheBusComponent implements OnInit {
  @ViewChildren (GheBusComponent) listBusSeat: QueryList<GheBusComponent>;
  mangGhe = [
  ...
  datghe(seat){
    ...
    if(seat.trangThai === true){
      ...
      this.danhSachGheDaDat.splice(this.danhSachGheDaDat.findIndex(item => item.SoGhe === seat.ghe.SoGhe), 1);
    }
    
  }
  huyGhe(seat){
    this.danhSachGheDaDat.splice(this.danhSachGheDaDat.findIndex(item => item.SoGhe === seat.SoGhe), 1);
    let cancelSeat = this.listBusSeat.find(item => item.seat.SoGhe === seat.SoGhe);
